...
function showProfileData() {
    const name = session.fullName || session.username || "StockFlow User";

    setText("dashUser", name);
    setText("sidebarUser", name);
    setText("sidebarRole", session.role || "Employee");
    ...
    setText("welcomeTitle", `Good afternoon, ${name}.`);

    setText("profileName", name);
    setText("profileUsername", session.username || "—");
    setText("profileEmail", session.email || "—");
    setText("profilePhone", session.phone || "—");
    setText("profileRole", session.role || "Employee");
    setText(
        "profileStatus",
        session.demo ? "DEMO — NOT VERIFIED" : "ACTIVE — VERIFIED"
    );
    ...
    const demoBanner = id("demoBanner");
    const profileNotice = id("profileDemoNotice");

    if (session.demo) {
        if (demoBanner) demoBanner.hidden = false;
        if (profileNotice) profileNotice.hidden = false;
    }
}
...
function startReverification() {
    ...
    if (!api) {
        alert("Google Apps Script URL is missing.");
        return;
    }
    ...
    fetch(api, {
        ...
        .then(result => {
            ...
            sessionStorage.setItem(
                "stockflow_demo_otp",
                window.STOCKFLOW_CONFIG?.DEMO_OTP || "123456"
            );
...
// Don't pretend unavailable modules work.
document.querySelectorAll(".not-connected-link").forEach(link => {
    link.addEventListener("click", event => {
        event.preventDefault();
        alert(
            "This module is currently NOT CONNECTED. " +
            "The phone-accessories inventory integration will be added later."
        );
        closeMenu();
    });
});
...
// Reverification
id("reverifyBtn")?.addEventListener("click", startReverification);
id("profileReverifyBtn")?.addEventListener("click", startReverification);
