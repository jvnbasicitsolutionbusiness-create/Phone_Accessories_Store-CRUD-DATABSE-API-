...
//
// AUTHENTICATION / OTP VERSION
// ------------------------------------------------------------
// Google Sheets = primary database
// Firebase       = required user + OTP mirror
//
// OTP:
// - Backend generates the real verification OTP.
// - OTP is stored in Google Sheets.
// - The SAME OTP is stored in Firebase.
// - The generated OTP is returned to the frontend so the
//   verification boxes can be automatically populated.
// - No fake/demo OTP is generated by the frontend.
// - No real Gmail/SMS provider is required.
// ============================================================
...
// ============================================================
//
// UID is different from numeric ID.
//
// Example:
//
// ID  = 1
// UID = sf_20260903153000123_A8F12C9B3D21
//
// ============================================================
...
//
// IMPORTANT:
// This does NOT reorder the user's current sheet.
//
// ID and UID are completely separate fields.
//
// ============================================================
...
// ============================================================
//
// This function creates the ONE actual OTP.
//
// The exact same code is:
// 1. saved in Google Sheets
// 2. saved in Firebase
// 3. returned to the frontend
//
// ============================================================
...
function createOTPForUser(
  ...
  try {
    ...
    if (
      ...
      if (
        firebaseOtpResult &&
        firebaseOtpResult.success === false
      ) {

        throw new Error(
          firebaseOtpResult.message ||
      ...
      throw new Error(
        "Firebase OTP storage function is unavailable."
      );
    ...
    throw new Error(
      "The verification OTP could not be stored in Firebase. " +
      sfErrorMessage(
        firebaseError
      )
    );
  ...
  return {
    ...
    // The backend-generated OTP.
    otp:
      code,

    message:
      "Verification OTP generated and stored successfully."
...
function registerUser(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Unable to create verification OTP."
      );
    ...
    try {
      ...
      if (
        ...
        if (
          firebaseUserResult &&
          firebaseUserResult.success === false
        ) {

          throw new Error(
            firebaseUserResult.message ||
            "Firebase user storage failed."
          );
        ...
        throw new Error(
          "Firebase user storage function is unavailable."
        );
      ...
      throw new Error(
        "The account was created in Google Sheets, but Firebase user storage failed. " +
        sfErrorMessage(
          firebaseError
        )
      );
    ...
    return {
      ...
      // The ACTUAL OTP generated by this request.
      otp:
        otpResult.otp,
...
function registerAdmin(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "Unable to create admin verification OTP."
      );
    ...
    try {
      ...
      throw new Error(
        "Firebase admin user storage failed. " +
        sfErrorMessage(
          firebaseError
        )
      );
    ...
    return {
      ...
      message:
        "Admin registration successful. Your verification OTP is ready."
...
function verifyOTP(
  ...
  try {
    ...
    console.warn(
      "Unable to remove Firebase OTP after verification:",
      firebaseOtpError
    );
...
function prepareOtp(
  ...
  if (
    ...
    return {
      ...
      // Existing backend-generated OTP.
      otp:
        existing.otp,

      message:
        "Existing verification OTP is ready."
